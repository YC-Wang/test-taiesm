...
! Yi-Chi (Aug 2025)
! - move triggering function to CAM7
! - change pcols to ncol, r8 to kind_physics
!---------------------------------------------------------------------------------
...
! +++ Yi-Chi: add omega for CIN calculation
               omega, errmsg  ,errflg)
!               errmsg  ,errflg)
!--- Yi-Chi
...
! +++ Yi-Chi
   real(r8), intent(in) :: omega(ncols,pver)        ! vertical velocity
! original pcols, change to ncols, ok? 
! --- Yi-Chi
...
!
   ...
   logical plgcthk(ncol)
   integer lfc(ncol)
   real(kind_phys) cthk, esat, qsat, hmxsat
   real(kind_phys) heo(ncol,pver)
   ! --- Yi-Chi
...
real(kind_phys),  intent(in)  :: slimsk  ! land mask
real(kind_phys), intent(in)  :: pdot    ! vertical velocity at cloud base
real(kind_phys), intent(out) :: cincr   ! cin criteria

real(kind_phys) w1l, w2l, w3l, w4l, w1s, w2s, w3s, w4s
real(kind_phys) w1, w2, w3, w4
real(kind_phys) val1, val2, tem, tem1
real(kind_phys) cincrmax, cincrmin
     cincrmax=180.
     cincrmin=120.
